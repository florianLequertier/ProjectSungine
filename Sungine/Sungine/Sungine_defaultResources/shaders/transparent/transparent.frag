...
uniform PointLight pointLight[MaxPointLightCount];
uniform PointLight spotLight[MaxSpotLightCount];
uniform PointLight directionalLight[MaxDirectionalLightCount];
...
void main(void)
{
    ...
    // Read gbuffer values
    vec3 normalBuffer = texture(Bump, In.Texcoord).rgb;
    ...
    {
        color.rgb += computePointLight( pointLight, p, n, diffuse, specular, specularPower * 100 ) / pointLightCount;
    }

    int spotLightCount = min(MaxSpotLightCount, SpotLightCount);
    for(int i = 0; i < spotLightCount ; ++i)
    {
        color.rgb += computeSpotLight( spotLight, p, n, diffuse, specular, specularPower * 100 ) / spotLightCount;
    }

    int directionalLightCount = min(MaxDirectionalLightCount, DirectionalLightCount);
    for(int i = 0; i < directionalLightCount; ++i)
    {
        color.rgb += computeDirectionalLight( directionalLight, p, n, diffuse, specular, specularPower * 100 ) / directionalLightCount;
    }

    Color = vec4(color, color.a * ceil(1.0 - depth)); // Transparency at infinity
